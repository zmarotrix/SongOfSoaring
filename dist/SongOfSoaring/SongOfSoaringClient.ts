...
const EMPTY_SONG_DATA: Buffer = Buffer.alloc(16, 0);
const SONG_NOTE_POINTER: number = 0x80121F38;
const SONG_LENGTH_POINTER: number = 0x80121F40;
const SONG_OF_SOARING: Buffer = Buffer.from([0x05, 0x0B, 0x0E, 0x05, 0x0B, 0x0E]);
...
export default class SongOfSoaringClient implements ISongOfSoaringClient {
    ...
    noteData: Buffer = Buffer.alloc(2, 0);
    owl!: Texture;
    map!: Texture;
    cursor!: Texture;
    none!: Texture;
    ...
    onViUpdate() {
        ...
        if (!this.songPlayed && this.core.OOT?.link.state === Z64.LinkState.OCARINA) {
        ...
        if(this.core.OOT?.link.state === Z64.LinkState.OCARINA || this.debug) {
